...
/*
 Added By Ahmet Kazım Günay
 
 Converting NSStringEncoding to C String representation
 */
const char *ConvertNSStringEncodingToEncodingString(NSStringEncoding stringEncoding) {
    ...
    if (CFStringIsEncodingAvailable(cfStringEncoding)) {
        ...
        CFStringRef cfStringEncodingName = CFStringConvertEncodingToIANACharSetName(cfStringEncoding);
        finalEnc = MYCFStringCopyUTF8String(cfStringEncodingName);
    }
    return finalEnc;
}
/*
 Converting a CFStringRef to char *
 http://stackoverflow.com/a/9166500/2317286
 */
...
+ (NSArray *)nodesForXPathQuery:(NSString *)query onHTML:(NSData *)htmlData namespacePrefix:(NSString *)namespacePrefix namespaceURI:(NSString *)namespaceURI {
    ...
    if (floor(NSFoundationVersionNumber) > NSFoundationVersionNumber_iOS_8_0) {
        ...
        finalEnc = ConvertNSStringEncodingToEncodingString(encoding);
    }
    
    xmlDocPtr doc = htmlReadMemory([htmlData bytes], (int)[htmlData length], NULL, finalEnc, HTML_PARSE_NOWARNING | HTML_PARSE_NOERROR);
